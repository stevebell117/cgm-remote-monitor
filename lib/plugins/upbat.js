...
function init() {
  ...
  upbat.analyzeData = function analyzeData (sbx) {
    ...
    function analyzeStatus (status) {
      ...
      if (battery || voltage) {
        ...
        if (battery <= prefs.warn && battery > prefs.urgent) {
          uploaderStatus.notification = levels.WARN;
        } else if (battery <= prefs.urgent) {
          uploaderStatus.notification = levels.URGENT;
        }
    ...
    if (min && min.uploader) {
      ...
      result.status = levels.toStatusClass(min.uploader.notification);
  ...
  upbat.checkNotifications = function checkNotifications(sbx) {
    ...
    if (prop.min && prop.min.notification && prop.min.notification >= levels.WARN) {
  ...
  function alexaUploaderBatteryHandler (next, slots, sbx) {
